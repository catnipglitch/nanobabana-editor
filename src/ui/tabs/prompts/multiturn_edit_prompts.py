"""
Multi-Turn Edit Tab Prompts

マルチターン編集タブで使用する全プロンプト定義
このファイルはmultiturn_edit_tabでのみ使用される（他タブとの共有は影響度が高いため行わない）
"""
...
# パターン4: 元の写真を維持（ポーズ変更なし）- 処理タイプ2用
# 参考訳：
# 入力画像1は人物のポートレートです。
# 人物のみを丁寧に切り抜き、背景を完全に取り除きなさい。
# **重要**: 人物の姿勢、構図、ポーズは元の写真のまま完全に保持すること。
# 体の再構築や生成拡張は一切行わないこと。
# ライティングは均一でフラットなスタジオ光に整え、影の方向性が強まる照明を避けること。
# 人物の特徴や衣服の色は変えず、広告合成に適した透明背景で仕上げること。
# 最終画像は出力サイズに合わせて高品質にアップスケールしなさい。
PRESERVE_POSE_PROMPT = (
    "Input Image 1 is a portrait of a person.  "
    "Extract the person cleanly and remove all background elements.  "
    "CRITICAL: Preserve the person's exact posture, composition, and pose from the original photograph. "
    "Do NOT reconstruct the body or perform any generative extension.  "
    "Adjust the lighting to a flat, even studio style, avoiding dramatic shadows or directional highlights.  "
    "Preserve the subject's features and clothing colors, and output a clean transparent background for easy compositing.  "
    "Finally, upscale the image to the required output resolution with high quality."
)
